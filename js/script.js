 ...
 *
 * お知らせを管理するクラスです。
 * notifications.csvのモデルです。
 */
var NotificationModel = function (data) {
  this.start = new Date(data[0].replace(/(\d{4})(\d{2})(\d{2})/, '$1/$2/$3')).getTime()
  this.end = new Date(data[1].replace(/(\d{4})(\d{2})(\d{2})/, '$1/$2/$3')).getTime()
  this.notification = data[2]
  this.color = data[3]

  this.needToShow = function () {
    var current = Date.now()
    if (!this.start && !this.end) {
      return false
    }
    var start = this.start ? this.start <= current : true
    var end = this.end ? current <= this.end : true
    return start && end
  }
  ...
  this.getNotificationElement = function () {
    ...
    p.textContent = this.notification
    p.style.color = this.color || '#000000'
    return p
  }
}
...
$(function () {
  ...
  function showNotification () {
    csvToArray('data/notifications.csv', function (data) {
      data.shift()
      for (var i in data) {
        notifications.push(new NotificationModel(data[i]))
      }
      var notificationElements = []
      notifications.forEach(function (notification) {
        if (notification.needToShow()) {
          notificationElements.push(notification.getNotificationElement())
        }
      })
      $('#notifications').append(notificationElements)
    })
  }
